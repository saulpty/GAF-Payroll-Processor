...
export default function AdminEmployees() {
  ...
  const handleToggle = async (emp: EmpRow, key: FlagKey) => {
    ...
    if ((key === 'active' || key === 'excluded_from_payroll') && newVal === false && key === 'active') {
  ...
  const filtered = emps.filter(e => {
    const matchSearch = !search.trim() || e.display_name.toLowerCase().includes(search.toLowerCase()) || e.teramind_email.toLowerCase().includes(search.toLowerCase());
    ...
    <div className="p-6">
      ...
      {showForm && editing && (
        <Card className="mb-5 border-blue-300 shadow-sm">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm">{editing.id ? `Editing: ${editing.display_name}` : 'New Employee'}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 gap-3 mb-4">
              ...
              <div>
                ...
                <select className="w-full border rounded px-2 py-1.5 text-sm"
                  ...
                  {(schedules as Schedule[]).map(s => <option key={s.id} value={s.id}>{s.schedule_name}</option>)}
                </select>
              </div>
            </div>
        ...
        <div className="flex rounded-lg border overflow-hidden text-xs">
          {(['active', 'inactive', 'all'] as const).map(f => (
            <button key={f} onClick={() => setFilterActive(f)}
              className={`px-3 py-2 capitalize border-r last:border-r-0 font-medium transition-colors ${filterActive === f ? 'bg-slate-800 text-white' : 'bg-white text-slate-600 hover:bg-slate-50'}`}>
              {f}
            </button>
          ))}
        </div>
      ...
      <div className="rounded-lg border overflow-auto shadow-sm">
        <table className="w-full text-xs border-collapse" style={{ minWidth: 900 }}>
          ...
          <tbody>
            {filtered.length === 0 && (
              <tr><td colSpan={9} className="px-4 py-8 text-center text-sm text-muted-foreground">No employees match your filter.</td></tr>
            )}
            {filtered.map(emp => (
              <tr key={emp.id} className={`border-b hover:bg-slate-50 transition-colors ${!emp.active ? 'opacity-50' : ''} ${emp.excluded_from_payroll ? 'bg-red-50/50' : ''}`}>
                ...
                <td className="px-3 py-2 border-r">{emp.schedule_name || <span className="text-slate-300 italic">none</span>}</td>
                {FLAG_META.map(f => (
                ...
                <td className="px-3 py-2 text-center">
                  <Button size="sm" variant="outline" className="text-xs h-6 px-2" onClick={() => handleEdit(emp)}>Edit</Button>
                </td>
              </tr>
            ))}
          </tbody>

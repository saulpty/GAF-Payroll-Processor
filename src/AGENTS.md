...
- **The run:** an operator uploads a Teramind activity export (CSV or XLSX) (or punches captured from the Teramind API — see *Teramind saved copy* below) on
...
Other: `src/app/components/TimeInput.tsx`, `src/app/components/EmployeeSearchInput.tsx` (global filter bar employee box with name suggestions; writes the exact display name so page filters keep their substring match), `src/app/pages/attendance/*` (eight

-- Redefine v_attendance_daily so that it filters on each employee's own schedule's
-- work_days instead of the hardcoded Monday–Friday EXTRACT(isodow) < 6 test.
--
-- Changes from 1781401300_fix_view_remove_dst_display_offset.sql:
--   1. base CTE: LEFT JOIN schedules s ON s.id = e.schedule_id
--      + COALESCE(NULLIF(TRIM(s.work_days), ''), 'Mon,Tue,Wed,Thu,Fri') AS work_days
--   2. Final WHERE: replace EXTRACT(isodow FROM work_date) < 6
--      with the literal-array test that maps ISO DOW 1–7 to Mon–Sun abbreviations.
--
-- The literal array is deterministic and matches DOW_ABBR in classificationEngine.ts.
-- TO_CHAR(work_date, 'Dy') is deliberately NOT used — locale-dependent.
-- work_days is used only inside the WHERE clause and does NOT appear in the output.
-- Output columns are unchanged: email, name, date, entry_time, status, bucket,
-- filed_gaf, minutes_late, period_name  (nine columns, same names, same order).
-- Rollback: re-run 1781401300_fix_view_remove_dst_display_offset.sql.

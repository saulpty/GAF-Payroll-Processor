-- Fix: to_timestamp fails with lowercase "am"/"pm" in entry_time.
-- Wrap entry_time_txt with UPPER() so "8:06 am" → "8:06 AM" before parsing.
...
parsed AS (
  SELECT
    ...
    CASE WHEN entry_time_txt IS NULL THEN NULL
         ELSE to_timestamp(UPPER(entry_time_txt), 'HH12:MI AM')::time END AS entry_t,

...
test('AL3: the row uses the shared parts and the same discount math as the save', () => {
  ...
  // AR-12 moved the maths into arDiscount.ts, shared with the commit confirmation.
  const maths = readFileSync('src/app/pages/action-required/arDiscount.ts', 'utf8');
  assert.match(row, /rowDiscount\(row, edit\)/);
  assert.match(maths, /computeDiscount\(\{/);
  assert.match(row, /const day = fmtDay\(row\.work_date\.slice\(0, 10\), THIS_YEAR\)/);
...
test('AL4: minutes always recomputed like the save; Paid only once impacts are chosen', () => {
  const row = readFileSync('src/app/pages/action-required/arDiscount.ts', 'utf8');

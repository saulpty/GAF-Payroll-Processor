...
--
-- migration_id: 1781290000_fix_view_date_as_text
-- name:         fix view date as text
-- applied_at:   2026-06-16 14:55:44
-- applied_by:   system
-- checksum:     76925199129c1393549ebe9a96b2c6f387526ed3d317aee044187e531ec495da
--
...
-- Drop and recreate v_attendance_daily so the date column changes from date -> varchar.
-- CREATE OR REPLACE cannot change column types, so we must DROP first.
...
SELECT DISTINCT ON (employee_id, work_date)
  email,
  name,
  to_char(work_date, 'YYYY-MM-DD')::varchar                AS date,

...
| D | Contract & milestone tracking (1 m/3 m/6 m/1 y/2 y, contract end) | ✅ built 2026-09-01, **released to prod 2026-09-08**. See `HANDOFF-2026-09-01.md`. No raise flag: the January review is company-wide, so milestones are tenure tracking |
| H | Disciplinary actions viewer — read-only view of the actions filed in the separate GAF Disciplinary Actions Form app, plus a Close case / Reopen write path | ✅ built 2026-09-07, **released to prod 2026-09-08**. See `HANDOFF-2026-09-07.md`. New scope, not on the original A–G list. 16 actions across 10 employees, all overdue. `CaseFile.tsx` is prop-driven so C reuses it unchanged |
| C | Employee 360 — one-employee view composing directory, contracts, PTO, requests, attendance | **next** — B, D and H are all built. `tenure.ts` is pure, `loadContractMilestones` takes optional `manager`/`employeeId`, and `CaseFile.tsx` takes its rows as props with `loadDisciplinaryActions` already accepting `employeeName` — so D and H both compose into it unchanged. C is composition work, not new data work |
| I | Attendance Reports tab — per-employee-day view of punches, the Absence/Tardiness form behind each day, and whether it was filed before the shift; flags unexplained absences | ✅ built 2026-09-07, **released to prod 2026-09-08**. See `HANDOFF-2026-09-07-attendance-reports.md`. New scope, not on the original A–G list. `attendanceReport.ts` is pure with 47 tests; all three loaders take optional `manager`, so it composes into C and wires into G unchanged |
| J | PTO tracker ↔ payroll reconciliation — "In payroll" column with cycle, actual first day off / return / days, Restore for withdrawn rows, no recording before the return date, FH counted in days, weekday date format, employee autocomplete | ✅ built 2026-09-09. See `HANDOFF-2026-09-09.md`. Follow-up: **backfill the August sheet's 2026 floating holidays as dated `pto_approvals` rows** (source `excel_import`, `leave_type 'floating_holiday'`) once the dates are known, then drop the `GREATEST(sheet, recorded)` in `loadPtoBalancesInputs` — the GREATEST under-counts when the sheet says 1 and a *different* FH is recorded later |

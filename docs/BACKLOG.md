...
| H | Disciplinary actions viewer — read-only view of the actions filed in the separate GAF Disciplinary Actions Form app, plus a Close case / Reopen write path | ✅ built 2026-09-07, **on `/dev/` only — not yet released**. See `HANDOFF-2026-09-07.md`. New scope, not on the original A–G list. 16 actions across 10 employees, all overdue. `CaseFile.tsx` is prop-driven so C reuses it unchanged |
...
| I | Attendance Reports tab — per-employee-day view of punches, the Absence/Tardiness form behind each day, and whether it was filed before the shift; flags unexplained absences | ✅ built 2026-09-07, **on `/dev/` only — not yet released**. See `HANDOFF-2026-09-07-attendance-reports.md`. New scope, not on the original A–G list. `attendanceReport.ts` is pure with 47 tests; all three loaders take optional `manager`, so it composes into C and wires into G unchanged |

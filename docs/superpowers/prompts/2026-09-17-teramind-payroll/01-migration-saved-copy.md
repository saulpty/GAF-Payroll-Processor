# Teramind saved copy — three new tables and one setting

Create **one** new file and apply it as a migration. **No other file may be created, modified or
deleted.** Never edit `ProcessPayroll.tsx`, `PayrollMaster.tsx`, `ActionRequired.tsx`, `classificationEngine.ts`, `AdminLookups.tsx`, `teramindParser.ts`, or anything under `src/components/ui/`.

## Why

Payroll punches will come from the Teramind API instead of an uploaded file. Everything pulled is
kept in our own database: an audit trail, a fast source for re-runs, and the only place where
manager scoping can be enforced. These tables are new and nothing reads them yet, so this change
cannot affect any existing page.

## The file — use this content exactly, character for character

### `src/migrations/1782003000_teramind_saved_copy.sql`
...
1. Only `src/migrations/1782003000_teramind_saved_copy.sql` was created; nothing else changed.
2. The migration applied without error.
3. `SELECT count(*) FROM teramind_agents`, `teramind_sessions` and `teramind_pull_log` each return 0.
4. `SELECT value FROM classification_config WHERE key = 'teramind_sync_every_minutes'` returns `180`.
5. `SELECT count(*) FROM payroll_entries` is unchanged.

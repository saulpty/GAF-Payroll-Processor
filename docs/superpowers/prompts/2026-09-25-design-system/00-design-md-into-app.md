# Design system 00: put the design rules inside the app

**Only these two files may change:**
1. **New** `src/DESIGN.md`, with exactly the content between the `~~~~` lines below, byte for byte.
2. `src/AGENTS.md`: add **one** numbered item, **7**, directly after item 6 of the
   `## Non-negotiables` list, in the same style (and change nothing else):

   ```
   7. **Read `src/DESIGN.md` before building or changing any screen's look.**
...
No other file may be touched: no page, component, action, lib, css, config or migration. This
prompt adds documentation only; nothing in the running app changes.
...
## Report
- The byte size of `src/DESIGN.md`.
- The exact item 7 you added to `src/AGENTS.md`, and its line number.

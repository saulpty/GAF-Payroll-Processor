# AR-12: The commit confirmation shows everything it writes, and works from the keyboard
...
1. `src/app/pages/action-required/arDiscount.ts` (**new file**)
2. `src/app/pages/action-required/ArConfirm.tsx` (replace the whole file)

No other file may be touched.

## Why (design review, 2026-09-25)
The confirm window is the last check before a payroll write, but it only showed Event 1 and Impact 1:
no Event 2, no Impact 2, no discount. It also had the old look (raw dates, blue text, "row(s) to GREEN"),
Esc did not close it and keyboard focus stayed on the table behind it.

Now it shows both event/impact pairs with the impact colour dots, the Discount per row (computed by the
new shared `rowDiscount`, the same maths the row and the save use), the total deducted, Warm styling,
`role="dialog"`, Esc to cancel and focus on the Commit button.

`arDiscount.ts` is new and nothing else uses it yet; ArRow switches to it in the next prompt.

### `src/app/pages/action-required/arDiscount.ts`

# AR-13: The day stays in view; In, Out and Notes get names
...
1. `src/app/pages/action-required/ArRow.tsx`
2. `src/app/components/TimeInput.tsx`

No other file may be touched. (`TimeInput` is also used by Payroll Master and Admin Schedules; the new
`ariaLabel` prop is optional, so they are unaffected.)

## Why (design review, 2026-09-25)
- The table is about 2,100 px wide. Scrolling right to Event / Impact hid the Date column, so you picked
  pay without seeing which day. The short date now sits under the employee name in the pinned column.
- The In, Out and Notes boxes had no name for screen readers once typed in; they now say e.g.
  "In, Ana López 2026-09-07". The "Not a real time" message is linked to its box.
- The row now uses the shared `rowDiscount` from `arDiscount.ts` (added in AR-12) instead of its own
  copy of the maths. Same results.

# 01b — Attendance → Today: show the "No Reports Yet" chip; Absence-with-reason-Sick reads as Sick
...
- `src/app/pages/attendance/TodayRow.tsx` — edit 1 below, nothing else
- `src/app/lib/activityDays.ts` — replace the WHOLE file with the code in edit 2, byte for byte

No other file may be touched.

## Edit 1 — TodayRow.tsx

In the Why column the chip is rendered only when `why && why.kind !== 'none'`. Remove the
`why.kind !== 'none'` condition: a chip of kind `none` (label `No Reports Yet`, tone `amber`)
is shown like any other chip, with the amber class from `WHY_CHIP_CLS`. Only
`why === null` / `undefined` renders the muted "—". `isOnLeave` still ignores kind `none`, so the
Status stays `No Records`.

## Edit 2 — activityDays.ts (verbatim; one behaviour change: a form whose *reason* says Sick is a Sick chip)

The Monday attendance board files sick days as type `Absence` with reason `Sick`. The only
difference from the current file is inside `whyFor`, in the `rr.form` branch. Replace the entire
file with exactly this:
...
1. Only the two files changed; `activityDays.ts` is under 15 KB and has no runtime imports.
2. A scheduled person with no records today shows Status `No Records` and Why `No Reports Yet` (amber).
3. Carlos Aloma (Absence form, reason Sick, 2026-09-18) shows `On Leave` and Why `Sick`.
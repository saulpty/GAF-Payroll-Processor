# 11b — Hub: the Edit form, and the Edit button in the action detail (part 2 of 2)

> **Goes into GAF Panama HR Hub** (the Hub), not the Disciplinary Actions Form.
> Run after 11: it uses `useSaveDisciplinaryEdit.ts` and `EditResultPanel.tsx`.

**Inside this project the code root *is* `src`, so `src/app/…` means `app/…`.**

**Only these 2 files may change. No other file may be touched.**

- New: `src/app/pages/disciplinary/EditActionForm.tsx`
- Edit: `src/app/pages/disciplinary/ActionDetail.tsx`

Do not touch `useSaveDisciplinaryEdit.ts`, `EditResultPanel.tsx`, `ActionPdfBar.tsx`, any
action, lib, `CaseFile.tsx`, `DisciplinaryTable.tsx`, any payroll page or
`src/components/ui/`. Both files stay under 15 KB. **Do not press Save on a real warning**:
it overwrites live data and emails the manager.

## Why

Tim and Saul edit a warning box by box: the same boxes the manager filled in on the form.
Dropdowns for level, final outcome and situation type; date pickers; text boxes for the four
incident questions (worded for the chosen situation, as on the PDF); evidence checkboxes.
**Employee, manager and ref cannot change**: to change who a warning is about, delete it and
file a new one.

Only `isDisciplinaryAdmin` (prompt 10) shows **Edit**; the database refuses anyone else anyway.

The form uses plain `<select>` and `<input type="checkbox">` on purpose (the shared Select
component cannot hold an empty "None" value). Dates stay `'YYYY-MM-DD'` strings and are
compared as strings; never `new Date(str)`.

## 1. New file `src/app/pages/disciplinary/EditActionForm.tsx` (verbatim)

Copy it exactly, character for character. `&middot;` and `&hellip;` are HTML entities on
purpose; keep them as written.
...
## 2. `src/app/pages/disciplinary/ActionDetail.tsx`

After prompt 10 this file already imports `useDisciplinaryAdmin` and `ActionPdfBar`, reads
`const { isDisciplinaryAdmin } = useDisciplinaryAdmin();`, and renders
`<ActionPdfBar action={action} />` directly before `{/* Facts */}`.

- Change `import { CheckCircle2, Loader2, Trash2, RotateCcw } from 'lucide-react';` to
  `import { CheckCircle2, Loader2, Trash2, RotateCcw, Pencil } from 'lucide-react';`
- Add `import EditActionForm from './EditActionForm';` below the `ActionPdfBar` import.
- Directly after `  const [restoring, setRestoring] = useState(false);` add
  `  const [editing, setEditing] = useState(false);`
- Replace `      <ActionPdfBar action={action} />` with:
  ```tsx
  ...
  ```
- Wrap the two existing blocks that follow it, the `{/* Facts */}` `<div>` and the
  `{/* Meta line */}` block, **unchanged**, in:
  ```tsx
      {editing ? (
        <EditActionForm
          action={action}
        ...
        <>
          {/* the existing Facts div and Meta line block, exactly as they are */}
        </>
      )}
  ```
- The status bars, `CloseCaseDialog` and `DeleteActionDialog` stay exactly as they are.
- Confirm every identifier used in the file is imported (`useState`, `Button`, `Pencil`,
  `EditActionForm`, `ActionPdfBar`, `useDisciplinaryAdmin`). The file stays under 15 KB.

## Acceptance (load `/disciplinary` and look; do not save)

1. `git status` would show exactly these 2 files.
2. **As Saul:** expand an action. Next to **Download PDF**, on the right, is **Edit**.
   Clicking it replaces the text with boxes, already filled with that warning's values: the
   two dates, level, outcome ("None" when empty), situation type, the four questions labelled
   as on the PDF, evidence ticks, and the four text boxes. The employee, manager and ref are
   shown as plain text at the top.
3. Changing the situation type changes the four question labels.
4. Clearing the document date disables **Save and email** and shows
   "Document date is required."
5. **Cancel** puts the original text back with nothing saved (the "Edited by" line does not
   appear).
6. A deleted action (Deleted filter) has **no Edit** button.
7. **View as a manager:** no Edit button anywhere.

## Report back

1. Both file paths and their sizes in bytes.
2. What you saw for checks 2 to 7.
3. Any line in `ActionDetail.tsx` that did not match the description above.

---

## Operator notes (Claude, not UIB)

- Byte-compare `EditActionForm.tsx` against the code block (8598 bytes).
- The first real save is plan verification step 4: on the test warning only, after Saul's
  go-ahead. Change two boxes, Save, then check: the green "Saved — updated PDF emailed to …"
  panel; Download updated PDF opens with the new text; the email arrives with the "UPDATED"
  subject at the manager, CC Saul, Tim and Marcela; after **Done**, the detail shows
  "Edited by saul.f@vitasyahc.com on …" and **Download original PDF** gives the unchanged
  first PDF.
- Plan verification 5 (non-admin UPDATE changes 0 rows) is run from the query runner.

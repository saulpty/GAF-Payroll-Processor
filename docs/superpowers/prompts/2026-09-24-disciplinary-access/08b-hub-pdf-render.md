# 08b — Hub: the PDF drawing, `buildDisciplinaryPdfBase64` (part 2 of 2)

> **Goes into GAF Panama HR Hub** (the Hub), not the Disciplinary Actions Form.
> Run after 08: it imports `strings.ts` from that prompt and needs `jspdf` installed.

**Inside this project the code root *is* `src`, so `src/app/…` means `app/…`.
Create the file at `app/lib/disciplinaryPdf/render.ts`, never under `src/src/`.**

**Only this 1 file may change. No other file may be touched.**

- New: `src/app/lib/disciplinaryPdf/render.ts`

Do not edit `strings.ts`, `disciplinaryOptions.ts`, `package.json` or anything else. Do not
wire this into any page yet. Do not touch `src/components/ui/` or any payroll file.

## Why

This is the second half of the PDF generator copied from the form app's
`src/app/utils/generatePdf.ts` (`buildPdf`, English, base64 mode). It draws exactly the same
page: same Letter size, margins, colours, header, 3×2 grid, warning box, sections, notice box,
signature lines and footer. It takes one row as loaded by `loadDisciplinaryActions`
(snake_case fields such as `employee_name`, `q_expected`, `evidence_types`) and returns the
PDF bytes as a base64 string. It never downloads anything and never touches React.

The code below is final. **Copy it exactly, character for character.** Do not reformat,
rename, reorder, merge the helper functions, or "tidy" the repeated `setFont`/`setFontSize`
calls: they are the drawing.

The imports from `./strings` are written **without** a `.ts` extension. Keep them that way.

The `\u2014` sequences are an em dash written as an escape on purpose (3 of them). Keep them
as the six characters `\u2014`; do not replace them with a typed dash.

## New file `src/app/lib/disciplinaryPdf/render.ts` (verbatim)
...
- Exactly one new file, `src/app/lib/disciplinaryPdf/render.ts`. Nothing else changed.
- It exports exactly one function, `buildDisciplinaryPdfBase64(row: PdfRow): string`.
- Its only imports are `jsPDF` from `'jspdf'` and the three names from `'./strings'`
  (plus the two types). Confirm every identifier used in the file is imported or defined in it.
- It contains exactly 3 occurrences of `\u2014` and no typed em dash.
- The file is under 15 KB.
- The app still builds and every existing page still loads (nothing calls it yet).

## Report back

1. The file path as created, and its size in bytes.
2. Confirm TypeScript accepts `import { jsPDF } from 'jspdf'` (the package from prompt 08).
...
After the export, byte-compare `render.ts` against the code block above. Expected size:
10347 bytes (LF line endings). The first real test of the drawing is prompt 10's Download PDF button on
a warning that has no stored PDF.

# Roster: split RosterTab.tsx (15,000 bytes, at the limit) — no visible change
...
A pure move so the next Roster change has room. **Only these three files may change** (all in
`src/app/pages/admin/employees/`):
- **New** `rosterTypes.ts`: `EmpRow`, `Schedule`, `EMPTY_EMP`, `FlagKey`, `FLAG_META`, moved as-is.
- **New** `RosterForm.tsx`: the Edit / Add form card, moved as-is (props instead of closures).
- `RosterTab.tsx`: whole file below; imports the two new files.

No other file may be touched (not `upsertEmployee.ts`, `updateEmployee.ts`, `MondayTab.tsx`,
`src/components/ui/*`). The page must look and behave exactly as before, including the
duplicate-email guard.

## `src/app/pages/admin/employees/rosterTypes.ts` (whole file)
...
## `src/app/pages/admin/employees/RosterForm.tsx` (whole file)
...
## `src/app/pages/admin/employees/RosterTab.tsx` (whole file)
...
## Report
- Byte size of the three files; confirm no other file changed and Admin → Employees → Roster
